                    ...
                    if '>' in i:
                        ...
                        for f in range(len(df.index)):
                            if df.loc[f]['Searched genes'] == number:
                                ...
                                ref_name = df.loc[f]['UNIPROT_prot_name']
...
def def_4(ref_list):
    ...
    with open('References.fasta', 'a') as output: 
        for j in ref_list:
            u = UniProt(verbose=False)
            result = u.search(ref_list[j], frmt='fasta')
            result_fasta = ''.join(result.split('\n')[1:])
            name = '>REF_'+ref_list[j]+' '+j
            print(name.replace(' ', '_'))
            print(name.replace(' ', '_')+'_'+str(len(result_fasta))+'_aa', file=output)
            print(result_fasta, file=output)
    print("---> В файл FASTA 'References.fasta' добавлены референсные последовательности из UniProt")
    print()
